...
export function orderFromRow(row = {}) {
  ...
  return {
    ...
    payment: {
      ...
      receiptEmailSentAt: payment.receipt_email_sent_at,
      receiptEmailStatus: payment.receipt_email_status,
      receiptEmailError: payment.receipt_email_error,
    },
    discountInPaise: Number(row.discount_in_paise || 0),
    couponCode: row.coupon_code,
    shippingChargeInPaise: Number(row.shipping_charge_in_paise || 0),
    taxInPaise: Number(row.tax_in_paise || 0),
  ...
  const rows = userId
    ? await sql`
        SELECT o.*, p.provider, p.razorpay_order_id, p.razorpay_payment_id, p.razorpay_signature,
          ...
          p.receipt_email_sent_at, p.receipt_email_status, p.receipt_email_error
        FROM orders o
...
async function hydrateOrder(row) {
  ...
  return orderFromRow({
    ...
    payment: {
      ...
      receipt_email_sent_at: row.receipt_email_sent_at,
      receipt_email_status: row.receipt_email_status,
      receipt_email_error: row.receipt_email_error,
    },
    items,
  });
}
